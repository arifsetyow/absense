<?php 
	...
	if(isset($_POST["addproduct"])) {
		...
		$location= $newFilename;
			  ...
			  $query = "insert into produk (idkategori, namaproduk, gambar, deskripsi, rate, hargabefore, hargaafter, hari, stok)
			  values('$idkategori','$namaproduk','$location','
			  ','$rate','$hargabefore','$hargaafter', '$hari', '$stok')";
...
<body>
  ...
  <?php
   ...
   $hasil=mysqli_query($conn,"SELECT * from kategori order by namakategori");
  ...
  ?>
        ...
        <div class="w3-col s6 w3-left">
            ...
            <form method="post" enctype="multipart/form-data" action="insert_aksi.php">
    ...
    <br>
                                ...
                                <div class="d-sm-flex justify-content-between align-items-right">
									...
									<button style="margin-bottom:20px" data-toggle="modal" data-target="#myModal" class="btn btn-info">(+) Tambah Produk</button>
  ...
  $brgs=mysqli_query($conn,"SELECT * from produk join kategori on produk.idkategori=kategori.idkategori where produk.idkategori='$id_kat' order by idproduk DESC");
    ...
    $brgs=mysqli_query($conn,"SELECT * from kategori k, produk p where k.idkategori=p.idkategori order by idproduk DESC");
  }else{
       $brgs="";
  }
											$no=1;
											while($p=mysqli_fetch_array($brgs)){
												...
												<tr>
													...
													<td><a href="https://www.google.com/search?q=<?php echo $p['namaproduk']; ?>&source=lnms&tbm=isch&sa=X&ved=2ahUKEwiN75rhjOD1AhVDkNgFHU33AhAQ_AUoA3oECAIQBQ&biw=1536&bih=746&dpr=1.25"target="_blank"><?php echo $p['namaproduk']?></a></td>
														<td><?php echo $p['namakategori'] ?></td>
													
														   <input type="hidden" name="kode_brg" value="<?php echo $p['idproduk']; ?>">
														<td><center><input style="  width: 30%; text-align: center;"type="text" name="stok[]" value="<?php echo $p['stok'] ?>"></center></td>
														<td><?php echo $p['stok'] ?></td>
														<td><input style="  width: 80%; text-align: center;" type="text" name="harga" value="<?php echo rupiah($p['hargaafter']) ?>"></td>
...
<input class="w3-check" type="checkbox" name="chkl[]" value="<?php echo $p['idproduk']?>">
