...
$new_date = date("d-m-Y", $timestamp);
                ...
                $brgs=mysqli_query($conn,"SELECT * from saldo join login on saldo.userid=login.userid where saldo.tanggal = '$new_date' group by saldo.id_saldo DESC");
            }else{

                                            $brgs=mysqli_query($conn,"SELECT * from saldo join login on saldo.userid=login.userid group by saldo.id_saldo DESC");
                                        }
                                            $no=1;
                                            while($p=mysqli_fetch_array($brgs)){
                                                ...
                                                <tr>
                                                    <td><?php echo $no++ ?></td>
 <td>#<?php echo $p['id_saldo'] ?></td>
                                                   
                                                   <td><form action="view_poto.php" method="post">
        <input type="hidden" name="id_saldo" value="<?php echo $p['id_saldo'] ; ?>">
        <button><img src="../transaksi/<?php echo $p['foto'] ;?>" style="width:42px;height:42px;"></button>
    
</form> </td>
                                                    <td><?php echo $p['nama']; ?><br>
                                                    (<?php 
                                                    $userid = $p['userid'];
                                                    ...
                                                    <td><?php echo rupiah($p['jumlah']) ?></td>
                                                     ...
                                                     <td><form action="verifikasi_saldo.php" method="POST" enctype="multipart/form-data">
    <input type="hidden" name="sisa_saldo" value="<?php echo $p['saldo'] ?>">
    <input type="hidden" name="saldo" value="<?php echo $p['jumlah'] ?>">
    <input type="hidden" name="id_saldo" value="<?php echo $p['id_saldo'] ?>">
    <input type="hidden" name="userid" value="<?php echo $p['userid'] ?>">
    <input type="submit" name="kirim" <?php 
   ...
   elseif ($d == 'DITOLAK'){
    ...
    echo "";
     ...
     ?> value="SAH" class="w3-button w3-large w3-green" />
     <br>
<input type="submit" name="tolak" <?php 
   ...
   if($d == 'SUDAH'){
    ...
    echo "";
     ...
     ?> value="TIDAK" class="w3-button w3-large w3-red" />
        </form></td>
