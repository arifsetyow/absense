                ...
                <div class="menu-inner">
                    <nav>
                        <ul class="metismenu" id="menu">
                            ...
                            <li  class="active">
                                <a href="swikhwan.php"><i class="ti-dashboard"></i><span>Kelola Pesanan</span></a>
                            </li>
                             <li >
                                <a href="swikhwanditerima.php"><i class="ti-dashboard"></i><span>Pesanan Diterima</span></a>
                            </li>
...
$new_date = date("Y-m-d", $timestamp);
            ...
            <option value="1">1sw</option>
            <option value="2">2sw</option>
            <option value="3">3sw</option>
            <br>
        </select>
        <br><br>
...
// Outputs: 31-03-2019
                ...
                $brgs=mysqli_query($conn,"SELECT * FROM tb_order where tgl_transaksi = '$new_date' AND verifikasi = 'SUDAH' AND status = 'DIKIRIM' AND jenjang_pesan = 'sw' AND jkl = 'Laki-laki' group by tb_order.id_order DESC");
            }else
...
$new_date = date("d-m-Y", $timestamp);
                ...
                $brgs=mysqli_query($conn,"SELECT * FROM tb_order where tgl_transaksi = '$new_date' AND verifikasi = 'SUDAH' AND status = 'DIKIRIM' AND jenjang_pesan = 'sw' AND jkl = 'Laki-laki' AND kelas_pesan = '$kelas_pesan' group by tb_order.id_order DESC");
...
$tanggal = date('d-m-Y');
                                            $brgs=mysqli_query($conn,"SELECT * FROM tb_order where tgl_transaksi = '$tanggal' AND verifikasi = 'SUDAH' AND status = 'DIKIRIM' AND jenjang_pesan = 'sw' AND jkl = 'Laki-laki' group by  tb_order.id_order DESC");
                                        }
                                            $no=1;
    ...
    <button style="margin-bottom:20px" data-toggle="modal" data-target="#myModal" class="w3-button w3-xlarge w3-GREEN"  <?php 
   $status = $p['status'];
   if ($status == 'DITERIMA') {
    echo "disabled";
   }  elseif ($status == 'batal')  {
   ...
   ?>>Pesanan Diterima</button></center></td>
                                                    ...
                                                    <td><?php echo $p['status'] ?></td>
                                                     ...
                                                     <td><form action="download1.php" method="POST" enctype="multipart/form-data">
                                                         <input type="hidden" name="id_order" value="<?php echo $p['id_order'] ?>">
     <input type="submit" name="kirim" value="Lihat Pesanan" style="margin-bottom:20px" class="w3-button w3-xlarge w3-blue"></center></td>
                                                </form></td>
